...
with col1:
    f = open("C:/Users/Asus/Desktop/Code/Gen AI/Google AI/keys/.gemini_api_key.txt")
    key = f.read()
    ...
    if st.button("Guess"):
        ...
        if canvas_result.image_data is not None:
            ...
            with st.chat_message("assistant"):
                st.write(f"The model guesses: {response.text}")
                #st.write(st.session_state['word'].pop())
                #st.write(response.text.lower())

            if st.session_state['word'][-1] == response.text.lower():
